...
class StopVMSSThread(threading.Thread):
    ...
    def run(self):
        print "Stopping " + self.vmss_name + " in resource group " + self.resource_group
        sys.stdout.flush()
...
def stop_vmss(resource_group, vmss_name):
    ...
    # Stop the VM
    vmss_stop = compute_client.virtual_machine_scale_sets.deallocate(resource_group, vmss_name)
    vmss_stop.wait()
...
if resource_group_name is None and vmss_name is None:
    ...
    vmss_detail = compute_client.virtual_machine_scale_sets.get(resource_group_name, vmss_name, expand='instanceView')
...
for group in groups:
    vmss_list = compute_client.virtual_machine_scale_sets.list(group.name)
    for vmss in vmss_list:
        vmss_detail = compute_client.virtual_machine_scale_sets.get(group.name, vmss.name, expand=None)
        if type(vmss_detail) == "VirtualMachineScaleSet":
            if vmss_detail.get_instance_view.statuses[1].code == 'PowerState/running':
